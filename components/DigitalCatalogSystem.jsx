...
const INITIAL_CATALOGS = [
  { id: 1, title: "製品総合カタログ 2025年版",        fileSize: "8.2 MB",  description: "全製品ラインナップを網羅した総合カタログです。最新モデルの仕様・価格表を含みます。" },
  { id: 2, title: "産業用ソリューション ガイド",        fileSize: "4.5 MB",  description: "製造・物流現場向けの導入事例と活用シナリオを詳しくご紹介します。" },
  { id: 3, title: "サービス・保守パッケージ一覧",       fileSize: "2.1 MB",  description: "アフターサポートプランの詳細と、各プランの比較表を掲載しています。" },
  { id: 4, title: "技術仕様書（エンジニア向け）",       fileSize: "12.0 MB", description: "API仕様・電気回路図・取付寸法図を含む詳細な技術資料です。" },
];
...
function recentCount(history, minutes) {
  const th = Date.now() - minutes * 60000;
  return history.filter(h => new Date(h.ts).getTime() >= th).length;
}

// 来場者ログ
function loadVisitorLog()  { return lsGet(STORAGE_VISITORS, { entries: [], resetAt: null }); }
function addVisitorEntry(log, companyName, catalogs) {
  const entry = { id: Date.now(), ts: new Date().toISOString(), companyName: companyName.trim(), catalogs };
...
function exportCSV(entries, eventName) {
  ...
  const rows = entries.map((e, i) =>
    [i + 1, formatJa(e.ts), `"${e.companyName || "(未記入)"}"`, `"${(e.catalogs || []).join(" / ")}"`].join(",")
  );
...
export default function DigitalCatalogSystem() {
  ...
  const [visitorCompany, setVisitorCompany] = useState(""); // 企業名入力

  // 管理者側
  const [infoSaved,   setInfoSaved]   = useState(false);
  const [showAddForm, setShowAddForm] = useState(false);
  const [newCatalog,  setNewCatalog]  = useState({ title: "", fileSize: "", description: "" });
  ...
  // 認証フォーカス
  useEffect(() => {
    if (showAuthModal) setTimeout(() => passwordRef.current?.focus(), 50);
  }, [showAuthModal]);
  ...
  const handleAuth = () => {
    if (passwordInput === ADMIN_PASSWORD) {
      ...
      setAccessLog(loadAccessLog()); setVisitorLog(loadVisitorLog());
      setMode("admin");
    } else {
  ...
  const handleLogout = () => {
    ...
    setNewCatalog({ title: "", fileSize: "", description: "" });
    setFormError(""); setShowResetAccess(false); setShowResetVisitor(false);
  };
  ...
  // ── 来場者：ダウンロード（企業名も記録） ──
  const handleDownload = (titles) => {
    if (visitorCompany.trim() || selectedIds.length > 0 || titles.length > 0) {
      // 企業名が空でも記録する（任意項目）
  ...
  const handleAddCatalog = () => {
    ...
    setCatalogs(prev => [...prev, { id: Date.now(), title: newCatalog.title, fileSize: newCatalog.fileSize || "—", description: newCatalog.description, downloadUrl: "#" }]);
    setNewCatalog({ title: "", fileSize: "", description: "" }); setFormError(""); setShowAddForm(false);
  };
  ...
  const btnPrimary = (extra = {}) => ({
    ...
    cursor: "pointer", fontFamily: F, ...extra,
  });
  const btnOutline = (extra = {}) => ({
    display: "flex", alignItems: "center", gap: "6px",
    padding: "7px 13px", border: "1.5px solid #0ea5e9", borderRadius: "8px",
    background: "#fff", color: "#0ea5e9", fontSize: "13px", fontWeight: "600",
    cursor: "pointer", fontFamily: F, ...extra,
  });
  const btnDanger = (extra = {}) => ({
    display: "flex", alignItems: "center", gap: "5px",
    padding: "6px 12px", border: "1px solid #fca5a5", borderRadius: "7px",
  ...
  return (
    <div style={{ fontFamily: F, minHeight: "100vh", background: "var(--color-background-tertiary)", color: "var(--color-text-primary)" }}>
      ...
      {showAuthModal && (
        <div onClick={() => setShowAuthModal(false)}
          ...
          <div onClick={e => e.stopPropagation()}
            className={authShake ? "shake-anim" : "modal-anim"}
            ...
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"1.5rem" }}>
              <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
                <div style={{ width:"40px", height:"40px", borderRadius:"10px", background:"#0f172a", display:"flex", alignItems:"center", justifyContent:"center" }}>
                  <ShieldCheck size={20} color="#38bdf8" />
                </div>
                <div>
                  <p style={{ margin:0, fontWeight:"700", fontSize:"15px" }}>管理者認証</p>
                  <p style={{ margin:0, fontSize:"12px", color:"var(--color-text-secondary)" }}>パスワードを入力してください</p>
                </div>
              </div>
            ...
            <div style={{ position:"relative", marginBottom: authError ? "8px" : "1.25rem" }}>
              <input ref={passwordRef} type={showPassword ? "text" : "password"} placeholder="パスワードを入力"
                value={passwordInput}
      ...
      {showResetAccess && (
        <ResetConfirmModal
          title="アクセス数をリセットしますか？"
          count={accessLog.totalCount}
          unit="アクセス"
          onCancel={() => setShowResetAccess(false)}
      ...
      {showResetVisitor && (
        <ResetConfirmModal
          title="来場者リストをリセットしますか？"
          count={visitorLog.entries.length}
          unit="件のレコード"
          onCancel={() => setShowResetVisitor(false)}
          onConfirm={handleResetVisitor}
        />
      )}
      ...
      {mode === "visitor" && (
        <div>
          ...
          <main style={{ maxWidth:"720px", margin:"0 auto", padding:"1.5rem" }}>
            ...
            <div style={{ marginBottom:"1.25rem", background:"var(--color-background-primary)", border:"0.5px solid var(--color-border-tertiary)", borderRadius:"12px", padding:"1rem 1.25rem" }}>
              <div style={{ display:"flex", alignItems:"center", gap:"8px", marginBottom:"8px" }}>
                ...
                <label style={{ fontWeight:"700", fontSize:"14px", color:"var(--color-text-primary)" }}>
                  ご所属の企業・組織名
                </label>
              ...
              <input
                type="text"
                placeholder="例：株式会社〇〇製作所（記入しなくてもダウンロードできます）"
              ...
              <p style={{ margin:"6px 0 0", fontSize:"12px", color:"var(--color-text-secondary)" }}>
                ご記入いただくと、後日担当者よりご連絡させていただく場合があります。
              </p>
            </div>
            ...
            <div style={{ display:"flex", flexWrap:"wrap", gap:"10px", alignItems:"center", marginBottom:"1.25rem" }}>
              ...
              {selectedIds.length > 0 && (
                <button
                  onClick={() => {
                    const titles = catalogs.filter(c => selectedIds.includes(c.id)).map(c => `・${c.title}`);
                    handleDownload(titles);
                  }}
                  style={btnPrimary({ boxShadow:"0 2px 8px rgba(14,165,233,.35)" })}
                >
              ...
              {catalogs.map(catalog => {
                ...
                return (
                  <div key={catalog.id} onClick={() => toggleSelect(catalog.id)}
                    ...
                    <button onClick={e => { e.stopPropagation(); handleDownload([`・${catalog.title}`]); }}
          ...
          <footer style={{ marginTop:"3rem", padding:"1.5rem 1.5rem 2rem", textAlign:"center", borderTop:"0.5px solid var(--color-border-tertiary)" }}>
            ...
            <button onClick={openAuthModal} title="" style={{ background:"none", border:"none", cursor:"pointer", padding:"6px", opacity:.15, color:"var(--color-text-secondary)", display:"inline-flex", alignItems:"center", justifyContent:"center", borderRadius:"6px", transition:"opacity .2s" }}
          ...
          <main style={{ maxWidth:"860px", margin:"0 auto", padding:"1.5rem" }}>
            ...
            <section style={{ marginBottom:"2rem" }}>
              <SectionTitle icon={<Users size={18} color="#0ea5e9"/>} label="QRアクセス集計">
                <button onClick={() => setShowResetAccess(true)} style={btnDanger()} className="hover-danger">
                  <RotateCcw size={12}/>カウントをリセット
                </button>
              </SectionTitle>
            ...
            <section style={{ marginBottom:"2rem" }}>
              <SectionTitle icon={<Building size={18} color="#0ea5e9"/>} label="来場者リスト">
                <div style={{ display:"flex", gap:"8px", alignItems:"center" }}>
                  <span style={{ background:"#e0f2fe", color:"#0369a1", fontSize:"12px", fontWeight:"700", padding:"2px 10px", borderRadius:"20px" }}>
                    {visitorLog.entries.length}件
                  </span>
                  {/* エクスポート */}
                  <button onClick={() => exportCSV(visitorLog.entries, eventInfo.eventName)} style={btnPrimary({ fontSize:"12px", padding:"6px 12px" })}>
                    <FileDown size={13}/>CSV出力
                  </button>
                  {/* リセット */}
                  <button onClick={() => setShowResetVisitor(true)} style={btnDanger()} className="hover-danger">
              ...
              {showVisitorList && (
                <div style={{ background:"var(--color-background-primary)", border:"0.5px solid var(--color-border-tertiary)", borderRadius:"12px", overflow:"hidden" }}>
                  {visitorLog.entries.length === 0 ? (
                    <div style={{ padding:"2.5rem", textAlign:"center", color:"var(--color-text-secondary)", fontSize:"14px" }}>
                      まだ記録がありません。来場者がダウンロードを行うと、ここに表示されます。
                    </div>
                  ) : (
                    <>
                      {/* テーブルヘッダー */}
                      <div style={{ display:"grid", gridTemplateColumns:"36px 1fr 1fr 2fr", gap:"0", padding:"8px 16px", background:"var(--color-background-secondary)", borderBottom:"0.5px solid var(--color-border-tertiary)", fontSize:"11px", fontWeight:"700", color:"var(--color-text-secondary)", letterSpacing:".05em" }}>
                        <span>#</span>
                        <span>日時</span>
                        <span>企業名・組織名</span>
                        <span>ダウンロードしたカタログ</span>
                      </div>
                      {/* テーブル行 */}
                      <div style={{ maxHeight:"420px", overflowY:"auto" }}>
                        {visitorLog.entries.map((entry, i) => (
                          <div key={entry.id} className="hover-row"
                            style={{ display:"grid", gridTemplateColumns:"36px 1fr 1fr 2fr", gap:"0", padding:"10px 16px", borderBottom:"0.5px solid var(--color-border-tertiary)", fontSize:"13px", alignItems:"center" }}>
                      ...
                      {/* フッター */}
                      <div style={{ padding:"8px 16px", background:"var(--color-background-secondary)", borderTop:"0.5px solid var(--color-border-tertiary)", fontSize:"12px", color:"var(--color-text-secondary)", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            ...
            <section style={{ marginBottom:"2rem" }}>
              ...
              <div style={{ background:"var(--color-background-primary)", border:"0.5px solid var(--color-border-tertiary)", borderRadius:"12px", padding:"1.5rem" }}>
                <Field label="会社名">
                  <input type="text" value={tempEventInfo.companyName}
                    onChange={e => setTempEventInfo(p => ({ ...p, companyName: e.target.value }))}
                    style={inputStyle()}/>
                </Field>
                <Field label="展示会名">
                  <input type="text" value={tempEventInfo.eventName}
                    onChange={e => setTempEventInfo(p => ({ ...p, eventName: e.target.value }))}
            ...
            <section>
              ...
              {showAddForm && (
                <div style={{ background:"#f0f9ff", border:"1.5px solid #7dd3fc", borderRadius:"12px", padding:"1.25rem", marginBottom:"1rem" }}>
                  ...
                  {formError && <p style={{ color:"#dc2626", fontSize:"13px", margin:"0 0 8px" }}>{formError}</p>}
                  <div style={{ display:"grid", gap:"10px" }}>
                    ...
                    <div>
                      <label style={{ fontSize:"12px", fontWeight:"600", color:"#0369a1", display:"block", marginBottom:"4px" }}>ファイルサイズ</label>
                  ...
                  <button onClick={handleAddCatalog} style={{ marginTop:"12px", display:"flex", alignItems:"center", gap:"6px", padding:"9px 18px", border:"none", borderRadius:"8px", background:"#0ea5e9", color:"#fff", fontSize:"13px", fontWeight:"700", cursor:"pointer", fontFamily:F }}>
                    <Plus size={14}/>追加する
                  </button>
                </div>
              )}
...
function SectionTitle({ icon, label, children }) {
  return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"1rem", paddingBottom:"8px", borderBottom:"2px solid #0ea5e9" }}>
      <div style={{ display:"flex", alignItems:"center", gap:"8px" }}>
        {icon}
        <h3 style={{ margin:0, fontSize:"15px", fontWeight:"700", color:"#0ea5e9" }}>{label}</h3>
      </div>
...
function inputStyle(extra = {}) {
  return {
    ...
    border:"1.5px solid var(--color-border-tertiary)", borderRadius:"8px",
    fontSize:"14px", fontFamily:"'Meiryo','Hiragino Kaku Gothic ProN','Noto Sans JP','Yu Gothic',sans-serif",
...
function ResetConfirmModal({ title, count, unit, onCancel, onConfirm }) {
  return (
    <div onClick={onCancel}
      style={{ position:"fixed", inset:0, background:"rgba(15,23,42,.65)", backdropFilter:"blur(4px)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:1000, padding:"1rem" }}>
      <div onClick={e => e.stopPropagation()} className="modal-anim"
        ...
        <div style={{ display:"flex", gap:"10px" }}>
          <button onClick={onCancel}
            style={{ flex:1, padding:"10px", border:"1.5px solid var(--color-border-tertiary)", borderRadius:"8px", background:"transparent", color:"var(--color-text-primary)", fontSize:"13px", fontWeight:"600", cursor:"pointer", fontFamily:"'Meiryo',sans-serif" }}>
            キャンセル
          </button>
          <button onClick={onConfirm}
            style={{ flex:1, padding:"10px", border:"none", borderRadius:"8px", background:"#ef4444", color:"#fff", fontSize:"13px", fontWeight:"700", cursor:"pointer", fontFamily:"'Meiryo',sans-serif", display:"flex", alignItems:"center", justifyContent:"center", gap:"6px" }}>
